...
shinyUI( fluidPage(
  ...
  sidebarLayout(
    sidebarPanel(
      tabsetPanel(
        tabPanel("Input Data",
          ...
          #uiOutput("x"),
          #uiOutput("y"),
          numericInput("dims", "Dims", value = 2),
          numericInput("perplexity", "Perplexity", value = 30),
          numericInput("max_iter", "Max iterations", value = 500),
          radioButtons("pca", "PCA", choices = c("yes" = TRUE, "no" = FALSE), selected = TRUE, inline = TRUE),
          uiOutput("factors"),
          helpText(a(href = "https://datashare.mpcdf.mpg.de/s/NGGKr08ueY2NEpa/download", "Example input")),
          hr(),
        ...
        tabPanel("Plot settings", 
          ...
          textInput("title", "Plot title", value="t-SNE"),
          ...
          radioButtons("showLegend", "Show legend", choices = c("yes" = TRUE, "no" = FALSE), selected = TRUE, inline = TRUE),
          hr()
        )
      )
    ),
    mainPanel(
      ...
      p("This App uses R's ",code('Rtsne'),"and", code('ggplot2'), " packages. For more information read the respective documentation on cran for",
        a("Rtsne", href = "https://cran.r-project.org/web/packages/Rtsne/Rtsne.pdf"),
        "and",
